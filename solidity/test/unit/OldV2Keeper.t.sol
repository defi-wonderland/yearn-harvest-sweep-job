...
import {OldV2KeeperJobForTest} from 'test/unit/ForTest/OldV2KeeperJobForTest.sol';
...
/**
 * @notice Unit test of the OLD V2 Keeper Job
 *
 * @dev This is ONLY for gas comparison purposes
 */

contract UnitOldV2Test is Test {
  /*///////////////////////////////////////////////////////////////
  ...
  OldV2KeeperJobForTest public keep3rJob;
  ...
  function setUp() public {
    ...
    keep3rJob = new OldV2KeeperJobForTest({
                _governor: governor,
   ...
   */
  function test_Oldstrategies_shouldReturnAllStrategies(uint256 _numberOfStrategiesToAdd) external {
   ...
   */
  function test_OldsetWorkCooldown_shouldSetCooldown(uint256 _cooldown) external {
   ...
   */
  function test_OldsetMechanicsRegistry_shouldSetRegistry(address _newRegistry) external {
    vm.prank(governor);
   ...
   */
  function test_OldgetCallCosts_shouldReturnZeroWhenNoRequiredAmount(address _randomAddress) external {
   ...
   */
  function test_Oldworkable_revertIfStragyDontExist(address _randomAddress) external {
    // Check: correct error?
   ...
   */
  function test_Oldworkable_shouldReturnTrueIfCooldownIsZero() external {
    // Add a strategy
    vm.prank(governor);
   ...
   */
  function test_Oldworkable_shouldReturnTrueIfCooldownHasExpired() external {
    // Add a strategy
    vm.prank(governor);
    keep3rJob.addStrategy(strategy, 0);
   ...
   */
  function test_Oldworkable_shouldReturnFalseIfCooldownHasNotExpired() external {
    vm.prank(governor);
    keep3rJob.addStrategy(strategy, 0);
   ...
   */
  function test_OldworkInternal_revertIfStrategyNotWorkable() external {
    vm.prank(governor);
    keep3rJob.addStrategy(strategy, 0);
   ...
   */
  function test_OldworkInternal_shouldUpdateLastWorkAtAndEmitEvent() external {
    vm.prank(governor);
    keep3rJob.addStrategy(strategy, 0);
   ...
   */
  function test_OldaddStrategy_shouldAddStrategyAndEmitEvent() external {
    vm.expectEmit(true, true, true, true, address(keep3rJob));
   ...
   */
  function test_OldaddStrategy_shouldNotAddTwice() external {
   ...
   */
  function test_OldaddStrategy_shouldSetRequiredAmount(uint256 _amount) external {
   ...
   */
  function test_OldaddStrategies_shouldAddAllStrategies(uint256 _numberOfStrategies) external {
   ...
   */
  function test_OldaddStrategies_shouldNotAddTwice() external {
   ...
   */
  function test_OldaddStrategies_shouldSetRequiredAmounts() external {
    address[] memory _strategies = new address[](2);
   ...
   */
  function test_OldaddStrategies_revertIfLengthsMismatch() external {
    address[] memory _strategies = new address[](2);
    _strategies[0] = address(uint160(69_696_969));
    _strategies[1] = address(uint160(69_696_969 * 2));
   ...
   */
  function test_OldremoveStrategy_shouldRemove(uint256 _strategyToRemove) external {
   ...
   */
  function test_OldremoveStrategy_shouldNotRemoveTwice() external {
    vm.startPrank(governor);
    keep3rJob.addStrategy(strategy, 0);
   ...
   */
  function test_OldupdateRequiredAmount_shouldUpdate(uint256 _initialAmount, uint256 _newAmount) external {
   ...
   */
  function test_OldupdateRequiredAmount_revertIfStrategyDontExist() external {
    vm.prank(governor);
   ...
   */
  function test_OldupdateRequiredAmounts_updateAmountsAndEmitEvent() external {
   ...
   */
  function test_OldupdateRequiredAmounts_revertIfLengthMismatch() external {
    address[] memory _strategies = new address[](2);
    _strategies[0] = address(uint160(69_696_969));
    _strategies[1] = address(uint160(69_696_969 * 2));
  ...
  /**
   * @notice Setting a new V2 Keeper works
   */
  function test_OldsetV2Keeper_shouldSet() external {
    vm.prank(governor);
