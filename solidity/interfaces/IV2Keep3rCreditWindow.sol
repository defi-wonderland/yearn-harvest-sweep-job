...
interface IV2Keep3rCreditWindow {
  ...
  /// @return sweepingPeriodStartAt     The timestamp at which the sweeping period starts, initialised as the deployment of this contract but should be adated
  ///                                   in the long run, as the whole period to cover will grow over time
  /// @return creditOptimisationWindow  The delay before the next period during which the credit optimisation is active
  function sweepingParams() external returns (uint128 sweepingPeriodStartAt, uint128 creditOptimisationWindow);
